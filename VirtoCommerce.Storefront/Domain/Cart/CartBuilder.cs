    ...
    public class CartBuilder : ICartBuilder
    {
        ...
        protected virtual async Task PrepareCartInternalAsync(ShoppingCart cart, Store store)
        {
            ...
            if (cart.Items.Any())
            {
                ...
                var products = await _catalogService.GetProductsAsync(productIds, ItemResponseGroup.ItemWithPrices | ItemResponseGroup.ItemWithDiscounts | ItemResponseGroup.Inventory | ItemResponseGroup.Outlines | ItemResponseGroup.ItemProperties);
